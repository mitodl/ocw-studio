...
def import_recent_videos(last_dt=None):
    ...
    if settings.DRIVE_VIDEO_UPLOADS_PARENT_FOLDER_ID:
        query += f" and ('{settings.DRIVE_VIDEO_UPLOADS_PARENT_FOLDER_ID}' in parents)"
...
def create_gdrive_folder_if_not_exists(website_short_id: str, website_name: str):
    ...
    query = f"(mimeType = 'application/vnd.google-apps.folder') and (name = '{website_short_id}' or name = '{website_name}')"
    ...
    if len(folders) == 0:
        ...
        service.files().create(
            supportsAllDrives=True, body=file_metadata, fields="id"
        ).execute()

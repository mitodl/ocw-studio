...
def _get_page_content_flag(request_data: dict, raw_site_config: dict) -> Optional[bool]:
    """Returns a value for WebsiteContent.is_page_content based on the request data and site config"""
    ...
    if site_config and config_item is not None:
        return site_config.is_page_content(config_item)
    return None


def _get_generated_filename(request_data: dict, website_pk: str) -> Optional[str]:
    """If 'filename' was not included in the request, returns an autogenerated value for WebsiteContent.filename"""
    title = request_data.get("title")
    if "filename" not in request_data and title is not None:
        filename_base = WebsiteContent.generate_filename(title)
        return get_valid_new_filename(
...
class WebsiteContentViewSet(
    ...
    def get_serializer_context(self):
        ...
        added_context = {"website_pk": website_pk}
        # Add 'is_page_content' value to context if page content is being created
        raw_site_config = website_qset["starter__config"] or {}
        page_content_flag = _get_page_content_flag(self.request.data, raw_site_config)
        if page_content_flag is not None:
            added_context["is_page_content"] = page_content_flag
        # Add generated filename to context if filename was not included in the request
        generated_filename = _get_generated_filename(self.request.data, website_pk)
        if generated_filename is not None:
            added_context["filename"] = generated_filename

...
@pytest.mark.parametrize("hide_download", [True, False])
def test_websites_endpoint_hide_download(
    settings, mocker, drf_client, permission_groups, hide_download
):
    """The hide_download endpoint should trigger removal task when hide_download is True"""
    mock_remove_task = mocker.patch(
        "content_sync.tasks.remove_download_content_for_site.delay"
    )
    settings.API_BEARER_TOKEN = "abc123"  # noqa: S105
    website = permission_groups.websites[0]

    # Create metadata content with hide_download setting
    WebsiteContentFactory.create(
        website=website,
        type=CONTENT_TYPE_METADATA,
        metadata={"hide_download": hide_download},
    )

    drf_client.credentials(HTTP_AUTHORIZATION=f"Bearer {settings.API_BEARER_TOKEN}")
    resp = drf_client.get(
        reverse("websites_api-hide-download", kwargs={"name": website.name})
    )

    assert resp.status_code == 200

    if hide_download:
        # Should trigger the removal task
        mock_remove_task.assert_called_once_with(website.name)
        # Should return empty response when hide_download is True
        assert resp.data == {}
    else:
        # Should not trigger the removal task
        mock_remove_task.assert_not_called()
        # Should return version timestamp when hide_download is False
        assert "version" in resp.data


def test_websites_endpoint_hide_download_denied(
    settings, drf_client, permission_groups
):
    """The hide_download endpoint should raise a permission error without a valid token"""
    settings.API_BEARER_TOKEN = "abc123"  # noqa: S105
    website = permission_groups.websites[0]
    drf_client.credentials(HTTP_AUTHORIZATION="Bearer wrong-token")
    resp = drf_client.get(
...
def test_websites_endpoint_hide_download_root_website(
    settings, mocker, drf_client, permission_groups
):
    """The hide_download endpoint should handle root website specially"""
    mock_remove_task = mocker.patch(
        "content_sync.tasks.remove_download_content_for_site.delay"
    )
    settings.API_BEARER_TOKEN = "abc123"  # noqa: S105
    settings.ROOT_WEBSITE_NAME = "root-site"

    # Create root website
    root_website = WebsiteFactory.create(name=settings.ROOT_WEBSITE_NAME)

    drf_client.credentials(HTTP_AUTHORIZATION=f"Bearer {settings.API_BEARER_TOKEN}")
    resp = drf_client.get(
        reverse("websites_api-hide-download", kwargs={"name": root_website.name})
    )

    assert resp.status_code == 200
    # Should not trigger the removal task for root website
    mock_remove_task.assert_not_called()
    # Should return version timestamp for root website
    assert "version" in resp.data

...
def test_websites_autogenerate_name_unicode(drf_client):
    """The autogenerated name should be compatible with unicode characters"""
    superuser = UserFactory.create(is_superuser=True)
    drf_client.force_login(superuser)
    starter = WebsiteStarterFactory.create(
        source=constants.STARTER_SOURCE_GITHUB,
    ...
    website_title = "テスト ページ"
    website_short_id = "テスト_ページ"
    slugified_title = slugify(website_title, allow_unicode=True)
    resp = drf_client.post(
        reverse("websites_api-list"),
        {"title": website_title, "short_id": website_short_id, "starter": starter.id},
